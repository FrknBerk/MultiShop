...
namespace MultiShop.Catalog.Services.StatisticServices
{
    public class StatisticService : IStatisticService
    {
        ...
        public async Task<decimal> GetProductAvgPrice()
        {
            ...
            var avgPrice = await _productCollection.Aggregate()
    .Group(p => p.ProductId, g => new { Product = g.Key, AvgPrice = g.Average(p => p.ProductPrice) })
    .FirstOrDefaultAsync();

            var values = "adasdasdasd";

            return (decimal)avgPrice.AvgPrice;
        }
